...
////ToDo: Comprueba primero con el mÈtodo del padre y luego si es prioritario. Es muy hardcoded. Se puede separar la comprobaciÛn de los prioritarios,
////en otro metÛdo y entonces es solo llamar a cada una de las funciones dependiendo de lo que necesites. Y se debe aÒadir un par·metro configurable
////desde el ini para indicar que comprueba primero. Ya que tenemos una lista con los tipos de m·quinas, ahora solo Normal o Reserved, pero en un
////futuro podrÌan ser m·s, se podrÌa hacer que para cada tipo de usuario se indique en una lista dÛnde puede ser alojado y en que orden.
////Seguramente se pueda separar el mÈtodo en varios ya que es muy largo y seguro que hay cÛdigo repetido de otras funciones.
...
CloudUser* CloudProvider_firstBestFit::findUserTypeById (std::string strUserId){
