        ...
        if (allUsersArriveAtOnce){
            ...
            for (int i=0; i<userInstances.size(); i++){
                ...
                if (userVm != nullptr)
                {
                    ...
                    EV_FATAL << "#___ini#" << m_nUsersSent << " " << (simTime().dbl()-m_dInitSim)/3600 <<"   \n" << endl;
                    m_nUsersSent++;
                }
            }
       }
...
//               {
                   ...
                   dRandom =  intervalBetweenUsers->doubleValue();
//               }while(nRandom>=0);
               ...
               if(pUserInstance!= nullptr)
               {
                   ...
                   scheduleAt (simTime().dbl()+dRandom, new cMessage (USER_GEN_MSG));

                   //TODO:Dejamos este msg o lo quitamos?
                   EV_FATAL << "#___ini#" << m_nUsersSent << " " << (simTime().dbl()-m_dInitSim)/3600 <<"   \n" << endl;
                   sendRequestMessage (userVm, toCloudProviderGate);
                   m_nUsersSent++;
                   delete (msg);
...
SM_UserVM* UserGenerator_simple::createVmRequest(CloudUserInstance* pUserInstance)
{
            ...
            if(userVm->hasResponse(i,nIndexRes))
            {
                ...
                if(pRes != nullptr)
                {
                    ...
                    if(bMaxStartTime_t1_active)
                    {
                        ...
                        {
                            userApp->createNewAppRequest(strService, strIp, strVmId, nStartRentTime);
                        }
                        else
                        {
...
void UserGenerator_simple::calculateStatistics()
{
    ...
    nIndex=1;
    nTotalTimeouts=0;
    dInitTime = dEndTime = dExecTime = dSubTime = dMaxSub = dTotalSub =  dMeanSub = 0;
    ...
    EV_FATAL << "#___t#" << dMaxSub << " "<< dMeanSub << " " << nTotalTimeouts << " " << dNoWaitUsers << " " <<  dWaitUsers << "   \n" << endl;
}
